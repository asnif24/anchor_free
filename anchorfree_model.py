...
from anchorfree_branch import ClassificationModel, RegressionModel
import losses
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # FPN return [P3_x, P4_x, P5_x, P6_x, P7_x]
        features = self.fpn([x2, x3, x4])

        # regressionModel out is [B, (H*W*n_anchors), 4]
        # regression = torch.cat([self.regressionModel(feature) for feature in features], dim=1)
        # out = [B, -1, 4]

        regression = [self.regressionModel(feature) for feature in features]

        # classificationModel out is [B, (H*W*n_anchors), n_classes]
